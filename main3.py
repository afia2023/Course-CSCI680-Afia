...
# Evaluating the model by predicting one token at a time and show only the first 10 results
def evaluate_model_single_token(model, test_methods, N, show_first_n=10, file=None):
    ...
    for method in test_methods[:100]:  # Automatically taking the first 100 test inputs
        tokens = ['<start>'] * (N-1) + tokenize(method) + ['<end>']
        for i in range(len(tokens) - N + 1):
            prefix = tuple(tokens[i:i+N-1])
            actual = tokens[i+N-1]
...
def main():
    ...
    # Using the full dataset for training, validation, and testing
    ...
    with open('output_results5.txt', 'w') as output_file:
        ...
        # Writing the best performing model to the file
        ...
        # Now using the best model for the 100-input token prediction task (test set)
